...
- 保持角色一致性，每句话都要符合你的人设
- effect 是可选的视觉特效，配合对话情绪使用，大多数回合设为 null。可选值：confetti（庆祝）、fireworks（烟花）、stars（星星）、hearts（爱心）、school-pride（双侧彩炮）、flash（闪光）、screen-shake（震动）、bomb（炸弹爆炸）、explosions（密集爆炸）、lightning（闪电）、vortex（漩涡）、glitch（故障）、rain（阴云下雨）、good-job（彩色文字庆祝）、panel-shake（对话框抖动）`;
