...
// ── RAG Suggest (HITL — blocks until user responds via addResult) ────────

type SuggestArgs = { queries?: string[]; defaultTopK?: number };
type SuggestResult = { action: string; query?: string; topK?: number };

export const SuggestSearchToolUI = makeAssistantToolUI<SuggestArgs, SuggestResult>({
	...
	return (
		<div className="mb-3 rounded-2xl border border-zinc-200/60 dark:border-zinc-700/50 bg-white dark:bg-zinc-800 overflow-hidden shadow-sm">
			...
			<div className="p-4 space-y-2">
				<div className="text-xs text-zinc-500 dark:text-zinc-400 mb-2">选择检索查询</div>
				{queries.map((q, i) => (
					<button
						key={q}
						type="button"
				...
				>
					<input
						type="text"
						value={custom}
						onChange={(e) => { setCustom(e.target.value); setSelected(queries.length); }}
			...
			<div className="px-4 pb-3">
				<div className="flex items-center justify-between mb-1">
					<span className="text-[10px] text-zinc-400 dark:text-zinc-500">检索数量</span>
					<span className="text-[10px] font-medium text-zinc-600 dark:text-zinc-300">{topK} 条</span>
				</div>
				<input
			...
			<div className="flex items-center gap-2 px-4 py-3 border-t border-zinc-100 dark:border-zinc-700/40">
				<button type="button"
					onClick={() => addResult({ action: "auto", message: "用户让你帮他选择，请自行决定最佳查询和参数来执行 rag_search。" } as SuggestResult)}
				...
				<button type="button" disabled={!canSubmit}
					onClick={() => addResult({ action: "confirm", query: activeQuery!, topK, message: `用户确认使用查询「${activeQuery}」检索 ${topK} 条结果。请调用 rag_search。` } as SuggestResult)}
					className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium transition ${canSubmit ? "bg-blue-500 hover:bg-blue-600 text-white cursor-pointer shadow-sm" : "bg-zinc-200 dark:bg-zinc-700 text-zinc-400 dark:text-zinc-500 cursor-not-allowed"}`}
				>
					<Search className="w-3 h-3" />
					确认搜索
				</button>
				<button type="button"
					onClick={() => addResult({ action: "skip", message: "用户不想被确认，请直接执行 rag_search。" } as SuggestResult)}
